...
export const bmadFairComparisonDesignPrompt = [
  ...
  "Before claiming implementation complete, create and validate an evidence packet with one explicit row per original Story Idea ID. Each row must include original Epic ID, Story Idea ID, title, value intent, expected behavior, mapped Delivery Story ID, implementation status (implemented, partial, deferred, rejected or out_of_scope), runtime artifacts, test artifacts and test IDs, latest test result, known limitations and any human decision required.",
  "The evidence packet must not only summarize Delivery Stories. It must prove that every original Story Idea is accounted for. Validation must fail if any baseline Story Idea is missing, any implemented Story Idea lacks Delivery Story mapping, runtime artifacts or test evidence, any scope-out item is marked implemented without explicit human approval, or evidence metadata is stale or inconsistent with the latest verification run.",
  "The final report must distinguish machine-verified evidence, AI self-review, human review still required and release approval status. It must not imply release approval unless explicit human approval is logged.",
