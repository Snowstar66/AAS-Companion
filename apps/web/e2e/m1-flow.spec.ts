...
const routes = {
  outcome: "/outcomes/outcome_demo_governance_gap",
  blockedStory: "/stories/story_demo_outcome_workspace",
  handoffStory: "/handoff/story_demo_execution_contract"
} as const;
...
test("M1 demo path covers login, ready outcome, and blocked story readiness", async ({ page }) => {
  await loginToDemoWorkspace(page);

  await page.goto("/");
  await expect(page.getByRole("heading", { name: "Home Dashboard" })).toBeVisible();

  await page.goto(routes.outcome);
