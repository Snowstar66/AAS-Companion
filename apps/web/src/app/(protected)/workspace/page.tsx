...
export default async function WorkspacePage({ searchParams }: WorkspacePageProps) {
  ...
  const outcomes = snapshot.data.organization.outcomes.filter((outcome) => {
    if (originFilter === "imported") {
      return outcome.originType === "imported";
    }

    if (originFilter === "native") {
      return outcome.originType !== "imported";
    }
      ...
      <section className="space-y-6">
        <div className="rounded-3xl border border-border/70 bg-[linear-gradient(135deg,rgba(255,255,255,0.96),rgba(241,246,252,0.92))] p-8 shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
          ...
          <p className="mt-3 max-w-3xl text-base leading-7 text-muted-foreground">
            Inspect Outcomes, Epics, and Stories in one shared governed model, with imported origin markers, readiness
            state badges, missing-link signals, and direct lineage back to Human Review when an object came from import.
          </p>
        </div>

        <div className="flex flex-wrap gap-2">
          <Button asChild variant={originFilter === "all" ? "default" : "secondary"}>
            <Link href="/workspace">All objects</Link>
          </Button>
          <Button asChild variant={originFilter === "imported" ? "default" : "secondary"}>
            <Link href="/workspace?origin=imported">Imported only</Link>
          </Button>
          <Button asChild variant={originFilter === "native" ? "default" : "secondary"}>
            <Link href="/workspace?origin=native">Native and seeded</Link>
          </Button>
        </div>
              ...
              <CardHeader>
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <CardDescription className="mt-2">{outcome.outcomeStatement ?? "Outcome statement is still missing."}</CardDescription>
                  </div>
                    ...
                    <div className="rounded-2xl border border-border/70 bg-muted/20 p-4" key={epic.id}>
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-2 text-sm leading-6 text-muted-foreground">{epic.purpose}</p>
                        </div>
                        <div className="flex flex-wrap gap-2">
