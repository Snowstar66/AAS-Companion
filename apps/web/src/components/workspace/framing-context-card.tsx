  ...
  return (
    <Card className="border-border/70 shadow-sm">
      ...
      <CardContent className="grid gap-4 md:grid-cols-3">
        {renderNodeLabel(outcome, t("Framing", "Framing"), t("Open", "Öppna"))}
        {epic
          ? renderNodeLabel(epic, t("Epic", "Epic"), t("Open", "Öppna"))
          : (
              <div className="rounded-2xl border border-dashed border-border/70 bg-muted/10 p-4">
                ...
                <p className="mt-2 text-sm font-medium text-foreground">{t("No Epic context selected yet.", "Ingen Epic-kontext är vald ännu.")}</p>
                <p className="mt-1 text-sm text-muted-foreground">{t("Stay in the current Framing to create or open one.", "Stanna i aktuell Framing för att skapa eller öppna en.")}</p>
              </div>
            )}
        {story
          ? renderNodeLabel(story, t("Story", "Story"), t("Open", "Öppna"))
          : (
              <div className="rounded-2xl border border-dashed border-border/70 bg-muted/10 p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">{t("Story", "Story")}</p>
                <p className="mt-2 text-sm font-medium text-foreground">{t("No Story context selected yet.", "Ingen Story-kontext är vald ännu.")}</p>
                <p className="mt-1 text-sm text-muted-foreground">{t("Only Stories under this Framing branch will appear here.", "Bara Stories under den här Framing-grenen visas här.")}</p>
              </div>
            )}
      </CardContent>
    </Card>
